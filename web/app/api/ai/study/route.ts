...
/**
 * A daily quiz is generated by this route but is not a StudyKind the
 * workspace serves: it has no setup screen, its options are fixed, and its
 * fingerprint is the day. Keeping it out of StudyKind keeps the workspace's
 * typing from silently widening to include a kind with no setup.
 */
type GenerationKind = StudyKind | "daily";

const KINDS = new Set<GenerationKind>(["summary", "flashcards", "practice", "daily"]);

/** Every daily quiz is five standard questions over the whole live Pot. */
const DAILY_OPTIONS: PracticeOptions = {
  questionCount: 5,
  difficulty: "standard",
  emphasis: "",
  sectionIds: [],
...
export async function POST(request: Request) {
  ...
  if (!potId || !KINDS.has(requestedKind as GenerationKind)) {
  ...
  const force = kind !== "daily" && body?.regenerate === true;
  ...
  // and the daily quiz takes none at all.
  const options = kind === "daily" ? DAILY_OPTIONS : normalizePracticeOptions(body?.options);
  ...
  // Two classmates opening the day's quiz at the same moment can both get past
  // the cache and spend a generation. Nothing prevents that fully without a
  // server-side lock, but checking the store once more here, after the quota
  // and before the work, closes most of the race: the loser finds the
  // winner's set and serves it instead.
  if (kind === "daily") {
    const { data: justStored } = await supabase
      .from("study_sets")
      .select("id, payload, model, created_at, secured")
      .eq("pot_id", potId)
      .eq("kind", kind)
    ...
    if (justStored) {
      return NextResponse.json(
        {
          result: justStored.payload,
          model: justStored.model,
          cached: true,
          generatedAt: justStored.created_at,
          studySetId: justStored.id,
          secured: justStored.secured === true,
        },
        { headers: NO_STORE },
      );
    }
  ...
  }).join("\n\n---\n\n").slice(0, kind === "practice" || kind === "daily" ? 24_000 : 60_000);
  ...
  try {
    const generated = await generateStructured<unknown>({
      ...
      schema: studySchemas[kind === "daily" ? "practice" : kind],
    });
    const result = normalizeStudyResult(
      kind === "daily" ? "practice" : kind,
      generated,
    ...
    // browser cannot know the key before the test is handed in. The daily
    // quiz is shaped the same way and split the same way, because the whole
    // class sees the same questions and nobody should see the key early.
    let memberPayload = result;
    let keys: Json | null = null;
    if (kind === "practice" || kind === "daily") {
    ...
    return NextResponse.json(
      {
        result: (kind === "practice" || kind === "daily") && stored ? memberPayload : result,
        model,
        cached: false,
        generatedAt: new Date().toISOString(),
        studySetId: saved.data ?? null,
        secured: (kind === "practice" || kind === "daily") && stored,
