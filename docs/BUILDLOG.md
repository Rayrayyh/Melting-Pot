...
## The front end, emptied of secrets (2026-09-12)

- A secrets audit of everything the browser receives found one real leak, in work from this same week: the Classroom settings card called `my_google_token` from the browser to show "Connected as", which put the Google access and refresh tokens into the page. The card now takes `connected` and `accountEmail` as props read on the server, and the built client chunks are grepped to prove no token, no model key name, and no Google or model endpoint ships. The residual is stated in decision 042: the token function remains callable by a signed-in user for their own row, because the server routes authenticate as that same user.
- The outside-stranger RLS suite grew with the schema: `game_rooms`, `game_players`, `game_answers`, `google_tokens` and `rate_limits` join the list of tables an anonymous client must read nothing from, and the new auth-gated functions (`record_study_run`, `submit_daily_quiz`, `create_game_room`, `game_state`, the three Google token functions) join the refusal checks.
- Everything else was checked and clean: the only public variables are the Supabase URL and publishable key and the two provider selectors; the model and Google client secrets live behind `server-only` imports; secured answer keys never leave the server or reach a client component; `public/` carries images, robots.txt and security.txt and nothing else.
